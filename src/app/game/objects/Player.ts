...
export class Player extends Phaser.GameObjects.Container {
  ...
  private _drawPlayer(chargeLevel = 0): void {
    ...
    gfx.fillStyle(COLORS.playerGlow, 0.25 + chargeLevel * 0.2);
    gfx.fillCircle(0, 0, glowR);

    // Mid glow ring
    gfx.fillStyle(COLORS.playerGlow, 0.4);
  ...
  private _drawTrail(): void {
    ...
    for (let i = 0; i < this.trail.length; i++) {
      ...
      if (t <= 0) continue;
      const r = GAME_CONFIG.physics.playerRadius * t * 0.8;
      this.trailGfx.fillStyle(COLORS.trail, t * 0.6);
      this.trailGfx.fillCircle(pt.x, pt.y, r);
    }
  }
  ...
  executeDash(dirX: number, dirY: number, speed: number): void {
    ...
    EventBus.emit(GameEvent.PLAYER_DASH, {
      ...
      speed,
      chargeLevel: speed / GAME_CONFIG.physics.dashMaxSpeed,
    });
  }
